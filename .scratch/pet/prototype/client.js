return {
  apply(ctx) {
    ...
    // ===== 动画纯逻辑内嵌（由 .scratch/pet/prototype/sync-inline.js 生成，sync.test.js 校验）=====

    // ==== inline-src: animation ====
...
// ==== inline-src-end: animation ====

    // ==== inline-src: multi-session ====
...
// ==== inline-src-end: multi-session ====
    ...
    async function selectPet(id) {
      try {
        const res = await host.call('pet/getPet', { id })
    ...
    function ensureInit() {
      ...
      initPromise = (async () => {
        try {
          const st = await host.call('pet/loadState', {})
    ...
    async function importPet(id, overwrite) {
      try {
        const res = await host.call('pet/importPet', { id, overwrite: overwrite === true })
        if (res !== null && typeof res === 'object' && res.ok) {
          await refreshPets()
    ...
    function PetView() {
      ...
      React.useEffect(() => {
        ...
        const fetchStatus = async () => {
          try {
            const res = await host.call('pet/getStatus', {})
        ...
        const dispose = timer.interval(fetchStatus, STATUS_POLL_MS)
        fetchStatus()
      ...
      React.useEffect(() => {
        if (timer === undefined) return
        return timer.interval(() => setTick((t) => t + 1), ANIM_TICK_MS)
      }, [])
    ...
    console.log('[pet] client half 已就绪')
  },
}
