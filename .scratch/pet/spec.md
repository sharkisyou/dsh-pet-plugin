...
2. 宠物包机制：兼容 Codex 宠物包格式（v1 + v2），DSH 宠物库 + 从本地路径导入
3. 状态浮层：宠物旁一行实时状态文本
...
- 设置页的宠物面板提供“导入宠物”按钮，点击后通过目录选择器选择本地宠物包目录。
- 导入 = 读源 pet.json + 图集 → 校验（图集 PNG/WebP、8 列网格）→ 只把必需的 `pet.json` 与图集复制到 `~/.dsh/pets/<id>/`（不整目录复制）。
- 导入性能：校验只读图集头部（~64 字节）解析宽高，不整张读入内存；批量导入用有限并发（3 个）处理，避免串行逐个导入过慢。
...
- 控制入口：设置页 `settings.section` 的宠物面板（空库、隐藏、无会话时也可打开），含唤醒/隐藏、宠物列表（按 displayName）、导入按钮；右下角与头部不设常驻入口。
...
- `settings.section`：宠物控制面板（唤醒/隐藏、选择、导入）。
